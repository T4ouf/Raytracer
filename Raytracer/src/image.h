...
/**
 * Image class :
 * Class that create and provide tools to manage images for our raytracer
 */
class Image {
protected:
    Color* _pixel;
...
public:
    ...
    // Normal accessors
    inline void put_pixel(int x, int y, Color c) { (*this)(x, y) = c; }
    inline Color get_pixel(int x, int y) const { return (*this)(x, y);}

    // Handier accessors
    // Usage: color = img(x,y);
    //        img(x,y) = color;
    inline const Color& operator()(int x, int y) const { return _pixel[index(x, y)]; }
    inline Color& operator()(int x, int y) { return _pixel[index(x, y)]; }

    // Normalized accessors, interval is (0...1, 0...1)
    inline const Color& colorAt(float x, float y) const { return _pixel[findex(x, y)]; }
